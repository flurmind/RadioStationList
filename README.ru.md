...
**П**лагин (техническое имя пакета — **RadioStationList**) для [Lyrion Music Server (LMS)](https://lyrion.org/) позволяет создавать и удобно управлять собственной коллекцией интернет-радиостанций: редактировать данные (включая логотипы), менять порядок, искать и добавлять новые станции из базы [Radio Browser](https://www.radio-browser.info) с фильтрацией по качеству потока и метаданным.
